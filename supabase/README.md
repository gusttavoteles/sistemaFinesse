...
Depois aplicar `supabase/migrations/20260918000300_mvp_operations.sql`. Essa migration adiciona o registro de contato de parcelas, a entrada financeira manual segura e o bucket privado `product-images`, com políticas subordinadas ao gate de master. O bucket não é público; as imagens são lidas pelo frontend por URL assinada. O frontend recebe uma imagem que o navegador consiga decodificar, normaliza-a para WebP com no máximo 2400 px no maior lado e até 5 MiB antes do upload. O arquivo original não é preservado. No download, a aplicação converte a imagem privada para JPG ou PNG conforme a escolha do usuário. A regra completa está na seção 20.9 da documentação central.
