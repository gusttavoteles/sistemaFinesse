...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const [productsResult, stockResult, categoriesResult] = await Promise.all([
      admin.from('produtos')
        .select('id,name,description,material,purity,sale_price,promotional_price,category_id,imagens_produtos(storage_path,is_cover,sort_order)')
        .eq('active', true)
        .order('name'),
