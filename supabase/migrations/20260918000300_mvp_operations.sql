-- Finesse Silver — operações do MVP que precisam permanecer no backend.
begin;
...
drop policy if exists "Finesse staff can read product images" on storage.objects;
create policy "Finesse staff can read product images"
on storage.objects for select to authenticated
using (bucket_id = 'product-images' and public.is_active_staff());

drop policy if exists "Finesse staff can upload product images" on storage.objects;
create policy "Finesse staff can upload product images"
on storage.objects for insert to authenticated
with check (bucket_id = 'product-images' and public.is_active_staff());

drop policy if exists "Finesse staff can update product images" on storage.objects;
create policy "Finesse staff can update product images"
