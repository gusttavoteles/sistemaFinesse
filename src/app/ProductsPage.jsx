...
export function ProductsPage() {
  ...
  async function save(event) {
    ...
    if (error) {
      ...
      setFeedback(imageError ? { type: 'error', message: imageError } : { type: 'success', message: 'Peça salva. O histórico dos pedidos foi preservado.' }); await load()
    }
    setSaving(false)
  }
  ...
  async function uploadImage(productId, file) {
    ...
    try {
      ...
      return `Peça salva, mas ${message.charAt(0).toLowerCase()}${message.slice(1)}`
    }
    ...
    if (upload.error) return 'Peça salva, mas a foto otimizada não foi enviada. Use Editar para tentar adicionar a foto novamente.'
    ...
    if (error) {
      ...
      return 'Peça salva, mas a foto não foi vinculada. Não cadastre outra peça; tente adicionar a foto em Editar.'
    }
    return null
  }
