...
export function ProductsPage() {
  ...
  async function save(event) {
    ...
    if (image && (!['image/jpeg', 'image/png', 'image/webp'].includes(image.type) || image.size > 5242880)) {
      setFeedback({ type: 'error', message: 'Use imagem JPEG, PNG ou WebP de até 5 MiB.' }); return
    }
    setSaving(true)
  ...
  async function uploadImage(productId, file) {
    ...
    if (upload.error) return 'Peça salva, mas a foto não foi enviada. Use Editar para tentar adicionar a foto novamente.'
    ...
    if (error) return 'Peça salva, mas a foto não foi vinculada. Não cadastre outra peça; tente adicionar a foto em Editar.'
    return null
  }
