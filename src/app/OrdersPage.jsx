...
export function OrdersPage() {
  ...
  async function load() {
    ...
    const [ordersResult, customersResult, productsResult, stockResult, accountsResult] = await Promise.all([
      supabase.from('orders').select('id,order_number,customer_id,status,payment_status,subtotal,shipping_amount,discount_amount,total_amount,notes,created_at,customer:customers(name),order_items(product_name_snapshot,quantity,unit_price,total_amount)').order('created_at', { ascending: false }),
      supabase.from('customers').select('id,name').eq('active', true).order('name'),
      supabase.from('products').select('id,name,sku,cost_price,sale_price,promotional_price').eq('active', true).order('name'),
      supabase.from('product_stock').select('id,current_stock'),
      supabase.from('financial_accounts').select('id,name').eq('active', true).order('name'),
    ])
  ...
  async function saveOrder(event) {
    ...
    const orderInsert = await supabase.from('orders').insert({ customer_id: form.customer_id || null, source: 'manual', notes: form.notes.trim() || null, shipping_amount: Number(form.shipping_amount || 0) }).select('id').single()
    ...
    if (Number(form.discount_amount || 0) > 0) await supabase.from('orders').update({ discount_amount: Number(form.discount_amount) }).eq('id', orderId)
    const { error } = await supabase.from('order_items').insert(items.map((item) => ({ order_id: orderId, product_id: item.product_id, product_name_snapshot: item.name, sku_snapshot: item.sku || null, quantity: item.quantity, unit_price: Number(item.unit_price), unit_cost_snapshot: Number(item.cost_price || 0), discount_amount: 0 })))
