...
const emptyForm = { name: '', phone: '', email: '', whatsapp_opt_in: false }
...
export function CustomersPage() {
  ...
  return <div className="page-content">
    <div className="page-heading"><div><span className="eyebrow">Relacionamento</span><h1>Clientes</h1><p>Cadastre quem compra e acompanha cobranças da loja.</p></div><button className="primary-button page-action" onClick={() => setShowForm((visible) => !visible)}>{showForm ? 'Fechar cadastro' : 'Novo cliente'} <span>＋</span></button></div>
    ...
    {showForm && <form className="panel form-panel customer-form" onSubmit={saveCustomer}><div className="panel-heading"><div><span className="eyebrow">Cadastro rápido</span><h2>Novo cliente</h2></div></div><div className="form-grid"><label>Nome completo<input name="name" value={form.name} onChange={updateField} required placeholder="Nome do cliente" /></label><label>Telefone<input name="phone" value={form.phone} onChange={updateField} placeholder="(00) 00000-0000" /></label><label>E-mail<input type="email" name="email" value={form.email} onChange={updateField} placeholder="cliente@email.com" /></label><label className="checkbox-label"><input type="checkbox" name="whatsapp_opt_in" checked={form.whatsapp_opt_in} onChange={updateField} /> Cliente autorizou contato pelo WhatsApp</label></div><div className="form-actions"><button type="button" className="secondary-button" onClick={() => setShowForm(false)}>Cancelar</button><button className="primary-button" disabled={saving}>{saving ? 'Salvando…' : 'Salvar cliente'}</button></div></form>}
    <section className="panel list-panel"><div className="panel-heading"><div><span className="eyebrow">Base ativa</span><h2>{customers.length} {customers.length === 1 ? 'cliente' : 'clientes'}</h2></div><label className="search-field"><span>⌕</span><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Buscar por nome, telefone ou e-mail" /></label></div>{loading ? <div className="loading-row">Carregando clientes…</div> : filteredCustomers.length === 0 ? <div className="empty-table"><span className="empty-icon">♧</span><strong>{search ? 'Nenhum cliente encontrado' : 'Nenhum cliente cadastrado'}</strong><p>{search ? 'Tente outro termo de busca.' : 'Cadastre o primeiro cliente para criar uma cobrança parcelada.'}</p></div> : <div className="data-table-wrap"><table className="data-table"><thead><tr><th>Cliente</th><th>Contato</th><th>WhatsApp</th><th>Cadastro</th></tr></thead><tbody>{filteredCustomers.map((customer) => <tr key={customer.id}><td><strong>{customer.name}</strong><small>{customer.email || 'Sem e-mail informado'}</small></td><td>{customer.phone || '—'}</td><td><span className={customer.whatsapp_opt_in ? 'status-pill success-pill' : 'status-pill muted-pill'}>{customer.whatsapp_opt_in ? 'Autorizado' : 'Não informado'}</span></td><td>{new Intl.DateTimeFormat('pt-BR').format(new Date(customer.created_at))}</td></tr>)}</tbody></table></div>}</section>
  </div>
}
