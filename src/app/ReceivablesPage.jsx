...
function messageFor(row) {
  ...
  return { phone, text: `Olá, ${name}! Tudo bem? Passando para lembrar que a parcela ${row.installment_number}/${row.installment_count}, no valor de ${money(row.amount)}, vence em ${date}. Se já realizou o pagamento, por favor desconsidere esta mensagem. Obrigada!` }
}
...
export function ReceivablesPage() {
  ...
  const visibleRows = useMemo(() => rows.filter((row) => statusFilter === 'open' ? ['pending', 'partially_paid', 'overdue'].includes(row.effective_status) : row.effective_status === statusFilter), [rows, statusFilter])
  const openTotal = useMemo(() => rows.filter((row) => ['pending', 'partially_paid', 'overdue'].includes(row.effective_status)).reduce((sum, row) => sum + Math.max(0, Number(row.amount) - Number(row.paid_amount || 0)), 0), [rows])
  ...
  async function registerPayment(event) {
    ...
    setSaving(true)
    setFeedback({ type: '', message: '' })
    const { error } = await supabase.rpc('record_installment_payment', { p_installment_id: payment.row.id, p_amount: Number(payment.amount), p_payment_method: payment.method, p_financial_account_id: payment.account_id, p_request_id: crypto.randomUUID(), p_paid_at: new Date().toISOString() })
    if (error) setFeedback({ type: 'error', message: 'Não foi possível registrar o recebimento. O valor pode exceder o saldo da parcela.' })
    else { setPayment(null); setFeedback({ type: 'success', message: 'Recebimento registrado no financeiro.' }); await loadData() }
    setSaving(false)
  }
  ...
  return <div className="page-content">
    ...
    <section className="metric-grid receivable-metrics"><article className="metric-card lavender"><div className="metric-top"><span>Em aberto</span><i>↗</i></div><strong>{money(openTotal)}</strong><small>Saldo de parcelas não pagas</small></article><article className="metric-card peach"><div className="metric-top"><span>Parcelas</span><i>◷</i></div><strong>{rows.filter((row) => ['pending', 'partially_paid', 'overdue'].includes(row.effective_status)).length}</strong><small>Em acompanhamento</small></article><article className="metric-card blue"><div className="metric-top"><span>Em atraso</span><i>!</i></div><strong>{rows.filter((row) => row.effective_status === 'overdue').length}</strong><small>Precisam de contato</small></article></section>
