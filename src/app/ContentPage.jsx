...
export function ContentPage({ session }) {
  ...
  async function load() {
    ...
    const [postsResult, productsResult] = await Promise.all([
      supabase.from('content_posts').select('id,product_id,caption,hashtags,scheduled_for,status,created_at,products(name)').order('created_at', { ascending: false }),
  ...
  async function save(event) {
    ...
    const { error } = await supabase.from('content_posts').insert({ product_id: form.product_id || null, caption: form.caption.trim() || null, hashtags: form.hashtags.trim() || null, scheduled_for: form.scheduled_for ? new Date(form.scheduled_for).toISOString() : null })
  ...
  async function updateStatus(post, status) {
    ...
    const { error } = await supabase.from('content_posts').update(payload).eq('id', post.id)
