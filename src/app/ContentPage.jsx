...
export function ContentPage({ session }) {
  ...
  async function downloadDay(day) {
    ...
    try {
      for (let index = 0; index < photos.length; index += 1) {
        ...
        link.download = `${day.key}-${String(index + 1).padStart(2, '0')}-${(post.produtos?.name || 'foto').replace(/[^a-z0-9]+/gi, '-').replace(/^-|-$/g, '').toLowerCase()}.${post.image_path.split('.').pop() || 'jpg'}`
  ...
  return <div className="page-content">
    ...
    <section className="panel weekly-planner"><div className="panel-heading"><div><span className="eyebrow">Agenda semanal</span><h2>Fotos da semana</h2><p className="field-help">A seleção é aleatória no banco. Cada foto só pode aparecer uma vez nesta semana.</p></div><label className="week-picker">Semana iniciando em<input type="date" value={weekStart} onChange={(event) => setWeekStart(mondayOf(event.target.value))} /></label></div>{loading ? <div className="loading-row">Carregando agenda…</div> : <div className="week-grid">{days.map((day) => <article className="day-card" key={day.key}><div className="day-card-heading"><div><strong>{day.label}</strong><small>{day.posts.length}/5 fotos</small></div><button className="text-button" disabled={saving || downloading === day.key || !day.posts.length} onClick={() => downloadDay(day)}>{downloading === day.key ? 'Baixando…' : 'Baixar fotos'}</button></div>{day.posts.length === 0 ? <div className="day-empty">Sem fotos programadas</div> : <div className="day-photo-list">{day.posts.map((post, index) => <div className="day-photo" key={post.id}>{post.image_path && (photoUrls[post.image_path] ? <img src={photoUrls[post.image_path]} alt="" /> : <div className="product-placeholder">◇</div>)}<span><strong>{post.produtos?.name || 'Foto sem produto'}</strong><small>{slotTimes[index] || formatScheduled(post.scheduled_for)} · {labels[post.status] || post.status}</small></span></div>)}</div>}</article>)}</div>}</section>
