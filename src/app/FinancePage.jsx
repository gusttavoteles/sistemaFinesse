...
export function FinancePage() {
  ...
  async function load() {
    ...
    const [accountsResult, categoriesResult, transactionResult] = await Promise.all([
      supabase.from('financial_accounts').select('id,name,type,institution,initial_balance,active').eq('active', true).order('name'),
      supabase.from('financial_categories').select('id,name,type').eq('active', true).order('name'),
      supabase.from('financial_transactions').select('id,type,direction,status,amount,transaction_date,description,financial_account_id,financial_accounts(name),financial_categories(name)').order('transaction_date', { ascending: false }).limit(100),
    ])
  ...
  const balances = useMemo(() => accounts.map((account) => {
    const total = transactions.filter((row) => row.financial_account_id === account.id).reduce((sum, row) => sum + (row.direction === 'in' ? Number(row.amount) : -Number(row.amount)), 0)
    return { ...account, balance: Number(account.initial_balance) + total }
  ...
  async function saveAccount(event) {
    ...
    const { error } = await supabase.from('financial_accounts').insert({ name: accountForm.name.trim(), type: accountForm.type, institution: accountForm.institution.trim() || null, initial_balance: Number(accountForm.initial_balance || 0) })
    if (error) setFeedback({ type: 'error', message: 'Não foi possível cadastrar a conta. Essa ação exige permissão de gestão.' })
    else { setShowAccount(false); setAccountForm({ name: '', type: 'bank', institution: '', initial_balance: '' }); setFeedback({ type: 'success', message: 'Conta financeira cadastrada.' }); await load() }
    setSaving(false)
  }
