...
export function FinancePage() {
  ...
  async function load() {
    ...
    const [accountsResult, categoriesResult, transactionResult] = await Promise.all([
      supabase.from('contas_financeiras').select('id,name,type,institution,initial_balance,active').eq('active', true).order('name'),
  ...
  async function saveAccount(event) {
    ...
    const { error } = await supabase.from('contas_financeiras').insert({ name: accountForm.name.trim(), type: accountForm.type, institution: accountForm.institution.trim() || null, initial_balance: Number(accountForm.initial_balance || 0) })
