...
export function EstimatedProfitPage() {
  ...
  return <div className="page-content">
    <div className="page-heading"><div><span className="eyebrow">Resultado das vendas</span><h1>Lucro estimado</h1><p>Veja o valor vendido, o custo histórico e o lucro estimado das peças vendidas.</p></div><button className="secondary-button" disabled={loading} onClick={() => void load()}>{loading ? 'Atualizando…' : 'Atualizar'}</button></div>
    ...
    <section className="metric-grid sold-profit-metrics">
      ...
      <article className="metric-card peach"><div className="metric-top"><span>Custo das peças</span><i>◇</i></div><strong>{loading || error ? '—' : money(totals.cost)}</strong><small>Snapshots dos custos na venda</small></article>
