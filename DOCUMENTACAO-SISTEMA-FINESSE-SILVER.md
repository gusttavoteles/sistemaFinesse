...
> **Revisão 38 — 23/09/2026:** definida a regra de futura integração com o site vitrine: o cadastro de produtos do Finesse será a fonte oficial e sincronizará estoque, preço e fotos para o site. O site não terá checkout: o botão de cada peça abrirá o WhatsApp com nome e valor; pedidos e estoque continuarão sendo registrados/ajustados no sistema. Produto ativo com estoque zero permanece visível como **Esgotada**; produto inativo ou inexistente no sistema não deve ser publicado. A integração ainda não foi implementada e depende de identificar plataforma/API do site.

> **Revisão 39 — 23/09/2026:** definida e implementada a unificação da vitrine `gusttavoteles/finessejoias` com o painel `gusttavoteles/sistemaFinesse` no projeto GitHub Pages do sistema. A raiz passa a ser a vitrine pública; o painel administrativo fica em `painel.html` e continua exigindo autenticação Supabase. O catálogo dinâmico usa a Edge Function `public-catalog`, que expõe somente campos aprovados para vitrine e URLs temporárias das fotos do bucket privado; não abre leitura anônima nas tabelas nem publica chave privilegiada. A vitrine foi refeita com layout responsivo, busca, filtros, preços atuais, disponibilidade e consulta via WhatsApp, sem checkout. Regras completas na seção 20.16. A função ainda precisa ser implantada/configurada no Supabase e o redirecionamento de Auth precisa incluir `painel.html` antes do deploy final.
> **Revisão 40 — 23/09/2026:** publicada a Edge Function `public-catalog` no projeto Supabase, com verificação JWT desativada somente para esse endpoint público e resposta limitada ao catálogo autorizado; validada a leitura de 33 peças reais no site local. O bucket de imagens continua privado e as imagens são servidas por URLs assinadas temporárias. No Supabase Auth, o Site URL e a lista de Redirect URLs agora incluem `https://gusttavoteles.github.io/sistemaFinesse/painel.html`, preservando o redirect local. A publicação do código/artefatos na branch `main` e a verificação do GitHub Pages ficam registradas após a conclusão do push.
> **Revisão 41 — 23/09/2026:** deploy concluído no GitHub Pages a partir do commit `9cb3b88` (workflow `pages build and deployment` com sucesso). A rota principal pública exibe a vitrine e carregou as 33 peças do Supabase; `/painel.html` exibe somente o login master quando não há sessão. A revisão visual foi feita por URL com cache-busting para evitar conteúdo antigo mantido no cache do navegador.
...
### 20.15 Regra de sincronização do catálogo (decisão original)

- **Fonte oficial:** produtos, fotos, preço e estoque mantidos no Finesse. O catálogo do site deve refletir os dados publicados pelo sistema; peça inativa ou sem cadastro correspondente não aparece na vitrine.
- **Disponibilidade:** produto ativo com estoque maior que zero aparece como disponível. Se o estoque chegar a zero, o site mantém a peça visível e exibe **Esgotada**; não permite tratá-la como disponível. Reposição lançada no sistema atualiza a disponibilidade da peça no site.
- **Contato e pedidos:** o site é uma vitrine sem compra/checkout online. O botão da peça abre o WhatsApp com o nome e o preço atual. Esse clique não cria pedido nem altera estoque. Os pedidos continuam sendo controlados no Finesse e o estoque é atualizado exclusivamente pelos fluxos de estoque/venda já existentes no sistema.
- **Direção da sincronização:** nesta decisão, a sincronização é do Finesse para o site. Não haverá importação automática de pedidos nem baixa de estoque iniciada pelo site.
- **Desativação:** desativar um produto no sistema remove-o da vitrine, preservando seu cadastro, imagens e histórico de pedidos no Finesse. Estoque zero, por si só, não desativa nem remove o produto.
- **Segurança:** nenhuma chave administrativa, chave `secret` ou `service_role` pode ser exposta no navegador/GitHub Pages. O bucket privado de origem não será tornado público por conveniência. A decisão inicial de transporte foi substituída pelo desenho registrado na seção 20.16; a regra de negócio desta seção continua vigente.

### 20.16 Vitrine pública e painel no mesmo GitHub Pages

- **Hospedagem e rotas:** o repositório `gusttavoteles/sistemaFinesse` publica dois documentos estáticos no mesmo site GitHub Pages: `/sistemaFinesse/` é a loja pública (`src/index.html`/`src/storefront-main.jsx`) e `/sistemaFinesse/painel.html` inicia o sistema interno (`src/painel.html`/`src/main.jsx`). O build Vite gera ambos; `scripts/publish-root.mjs` publica os dois HTMLs e os assets necessários na raiz versionada. O site antigo `gusttavoteles/finessejoias` permanece como origem legada das imagens editoriais referenciadas pela vitrine enquanto elas não forem migradas.
- **Separação é funcional, não sigilo por URL:** a vitrine não mostra um link de acesso ao painel e o HTML do painel solicita `noindex`, mas o nome `painel.html` não é uma barreira de segurança. Quem abrir a URL verá no máximo a tela de autenticação. O painel só mostra dados após sessão válida; as tabelas e operações permanecem submetidas às políticas RLS/RPC e aos dois masters autorizados. O mecanismo de confiança não é esconder a URL.
- **Fonte e sincronização:** catálogo, estoque e preço são lidos do Finesse em tempo de execução pela função `supabase/functions/public-catalog/index.ts`. O frontend público não consulta diretamente as tabelas de negócio. Produtos `active = true` entram na resposta; `active = false` não. Estoque maior que zero gera status **Disponível**; estoque igual/menor que zero gera **Esgotada**, mas mantém o produto no catálogo. A página não cria pedido, não reserva, não altera estoque nem oferece carrinho/checkout. Para valor, usa preço promocional positivo quando houver; senão, preço de venda atual. A mensagem individual do WhatsApp contém nome e preço vigentes; para item esgotado, pergunta sobre reposição.
- **Contrato de dados público e mínimo:** a resposta contém somente `id`, `name`, `description`, `material`, `purity`, nome da categoria, preço efetivo, quantidade atual, URL temporária da foto de capa e horário de geração. Não pode incluir custo de aquisição, SKU interno, fornecedor, clientes, compras, recebíveis, transações, auditoria ou dados de autenticação. A seleção de colunas da Edge Function deve ser revisada sempre que for alterada. A função prefere a chave privilegiada padrão em `SUPABASE_SECRET_KEYS` (mapa JSON do runtime) e usa `SUPABASE_SERVICE_ROLE_KEY` somente como compatibilidade legada; ambas ficam apenas no runtime Supabase e nunca vão para `VITE_*`, navegador, HTML ou repositório.
- **Fotos:** o bucket `product-images` permanece privado. A Edge Function cria URLs assinadas de leitura com validade de uma hora apenas para a imagem de capa de produto ativo. O site não ganha permissão de listar, enviar, atualizar ou excluir objetos do bucket. A publicação da URL é temporária; o navegador busca um catálogo novo ao carregar a página. Fotos editoriais de campanha atualmente vêm do repositório público da vitrine antiga e podem ser movidas para ativos próprios do novo repositório em uma revisão posterior.
- **Disponibilidade da função:** `supabase/config.toml` define `verify_jwt = false` apenas para `public-catalog`, pois a consulta da vitrine é pública; a função aceita somente `GET` e `OPTIONS`, consulta com acesso de servidor e deve devolver somente o contrato mínimo acima. CORS permite o uso do domínio GitHub Pages e não substitui controle de acesso. Respostas HTTP recebem cache curto de 60 segundos. A função não aceita instruções de consulta vindas do visitante.
- **Tela e UX:** a nova vitrine apresenta navegação compacta, chamada principal, destaques editoriais, filtros por categoria, pesquisa, preço, etiqueta de disponibilidade, estados de carregamento/erro/vazio, chamadas de WhatsApp e layout adaptado a celular. A vitrine usa produtos reais do sistema; não mantém preços nem estoque duplicados em HTML. Sem a Edge Function implantada/configurada ou sem credenciais públicas corretas de build, mostra um aviso e oferece atendimento, em vez de inventar produtos/disponibilidade.
- **WhatsApp:** o número mantido é `5562982593182`, reaproveitado da configuração do site anterior. Antes da publicação, o proprietário deve confirmar que este é o número comercial atual. O navegador monta a URL `wa.me` com texto UTF-8; o WhatsApp é aberto pelo visitante. Nenhuma mensagem é enviada automaticamente.
- **Estado de implantação (23/09/2026):** a Edge Function `public-catalog` está implantada, e o domínio publicado carregou 33 itens reais do Supabase; a configuração de segredo permaneceu somente no runtime Supabase. O Site URL e Redirect URL do painel foram configurados no Auth. Commit `9cb3b88` foi publicado na branch `main`; o workflow de GitHub Pages concluiu com sucesso. A rota principal foi validada como vitrine pública e `painel.html` como tela de login restrita.
- **Testes e arquivos:** antes de cada publicação, executar `npm test` e `npm run build`, confirmar que `dist/index.html` é a loja e `dist/painel.html` é o painel, e que a publicação copia os dois. Implementação visual em `src/storefront/Storefront.jsx`, `src/storefront-main.jsx`, `src/storefront.css`; chamada pública em `supabase/functions/public-catalog/index.ts`; entradas e deploy em `src/index.html`, `src/painel.html`, `vite.config.js`, `scripts/publish-root.mjs`; destino de autenticação em `src/lib/supabase.js`. Qualquer mudança no contrato público, regras de esgotado, preço/WhatsApp, rotas, fotos ou separação de acesso deve atualizar esta seção antes do código.
- **Visibilidade do repositório:** na inspeção de 23/09/2026, ambos os repositórios GitHub informados retornaram visibilidade pública. A proteção dos dados do painel não depende dessa visibilidade e depende de Auth/RLS; se o requisito for também manter o código-fonte privado, a visibilidade do repositório precisa ser alterada nas configurações do GitHub e a disponibilidade do Pages precisa ser reconfirmada. A URL do painel nunca deve ser descrita como senha ou segredo.
